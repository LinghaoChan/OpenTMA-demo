...
def get_mean_std(phase, cfg, dataset_name):
    ...
    assert name in ["t2m", "kit", "motionx"]
...
def get_WordVectorizer(cfg, phase, dataset_name):
    ...
    if phase not in ["text_only"]:
        if dataset_name.lower() in ["motionx", "humanml3d", "kit"]:
...
def get_collate_fn(name, cfg, phase="train"):
    if name.lower() in ["humanml3d", "kit", "motionx"]:
...
motion_subdir = {"humanml3d": "new_joint_vecs", "kit": "new_joint_vecs", "motionx": "motion_data"}
